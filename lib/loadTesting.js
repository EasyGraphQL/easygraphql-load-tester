...
class LoadTesting {
  ...
  artillery (options = {}) {
    const { customQueries, selectedQueries, queryFile, queryFilePath, withMutations } = options
    ...
    if (queryFile) {
      ...
      const filePath = queryFilePath ? path.join(queryFilePath, queryFileName) : path.join(path.resolve(), queryFileName)
  ...
  k6 (fileName, options = {}) {
    ...
    const k6Process = spawn('k6 run', [fileName], {
      stdio: 'inherit',
      shell: true,
      env: {
        VUS: selectedVus,
